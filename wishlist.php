<?php
    ...
    if (isset($_POST['delete_item'])) {
        ...
        $varify_delete_items = $conn->prepare("SELECT * FROM `wishlist` WHERE wishlist_id = ?");
        $varify_delete_items->execute([$wishlist_id]);

        if ($varify_delete_items->rowCount()>0) {
            $delete_wishlist_id = $conn->prepare("DELETE FROM `wishlist` WHERE wishlist_id = ?");
            $delete_wishlist_id->execute([$wishlist_id]);
            $success_msg[] = "wishlist item delete successfully";
        }else{
            $warning_msg[] = "wishlist item alredy deleted";
        }    
        }
